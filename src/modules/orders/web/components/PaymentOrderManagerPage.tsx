...
export const PaymentOrderManagerPage = () => {
  ...
  const calculateCost = (orderDetail: Product[]) => {
  ...
  }, [idOrder, toggleReload]);
  useEffect(() => {
    if (orderDetail) {
      calculateCost(orderDetail.productos);
    }
  }, [orderDetail]);
  return (
    <AppPageTransition>
      <>
        {orderDetail?.pagado === true ? (
          <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-green-100 to-green-200 px-4">
            ...
            >
              ...
              >
                <CheckCircle className="w-24 h-24 text-[#05DF72]" />
              </motion.div>
              ...
              {idOrder && (
                <p className="text-sm text-gray-500 mt-2">
                  Número de orden:{" "}
                  <span className="font-medium">{idOrder}</span>
                </p>
              )}
              ...
              whileTap={{ scale: 0.95 }}
              // onClick={onContinue}
              className="mt-8 w-full py-3 bg-green-500 text-white text-lg font-medium rounded-xl shadow hover:bg-green-600 transition"
            >
              Continuar
            </motion.button> */}
            </motion.div>
              ...
              <Card className="bg-warn-50">
                ...
                <CardBody className="flex flex-col items-start justify-start gap-3">
                  {orderDetail && orderDetail.productos.length > 0 && (
                    <>
                      {orderDetail.productos.map((item, index) => (
